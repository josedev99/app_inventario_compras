...
import FormSldModal from './components/FormSldModal';
...
export default function Ingreso({ auth, productos }) {
    ...
    const fetchIngresoInv = async (page = 1, perPage = 10, search = '') => {
        ...
        try {
            const response = await axios.get(route('inv.getSalidaMov'), {
                params: {
                    draw: 1,
                    start: start,
                    length: perPage,
            ...
            setMovimientoSalida(response.data.data);
    ...
    const showPDF = (codigo) => {
        window.open(route('inv.salida.pdf', btoa(codigo)), '_blank');
    }

    const columns = [
        { name: '#', selector: (_, index) => movimientoSalida.length - index, sortable: true, width: '5%', center: true },
        { name: 'Fecha', selector: row => row.fecha, sortable: true, width: '15%', center: true },
        { name: 'Código salida', selector: row => row.codigo, sortable: true, width: '15%', center: true },
    ...
    return (
        <AuthenticatedLayout user={auth.user} sidebar={<Sidebar />} header={<Nav />}>
            ...
            <FormSldModal title="Salida de productoStocks a inventario" setReloadDt={setReloadDt} dataProducto={productos} showModal={showModal} setShowModal={setShowModal} />
            ...
            <Card>
                <Card.Header className='d-flex justify-content-between align-items-center'>
                    <Button onClick={() => {
                        ...
                        <i className="bi bi-plus-circle"></i> Salida
                    </Button>
                    <input
                        type="text"
